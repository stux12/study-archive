...
본문에 아래 블록을 그대로 쓰면 사이트에서 시각자료로 렌더된다. 자세한 정책은
[콘텐츠·시각자료 정책](content-visual-policy.md) 참고.
...
- **다이어그램**: `/code` 코드 블록을 만들고 **언어를 `Mermaid`로** 지정 → 플로우차트·시퀀스 등으로 렌더.
  ...
  ```
- **콜아웃**: `/callout` 블록. 아이콘(이모지)과 색이 유지된다(초록=팁, 빨강·주황=주의, 그 외=참고).
- **접기 토글**: `/toggle` 블록. 제목이 요약, 안쪽이 접히는 내용 → 복습 질문의 답 가리기에 사용.
- **표**: `/table` 블록. 첫 행을 헤더로 두면 표 머리로 인식된다.
- **이미지**: 이미지 붙여넣기/업로드. 동기화 때 **저장소로 내려받아** 저장하므로 링크가 깨지지 않는다. 캡션을 달면 그림 설명으로 표시된다.
- **인라인 서식**: 굵게·기울임·취소선·인라인 코드·링크는 그대로 반영된다.
